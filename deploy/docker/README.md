...
|---|---|---|
| `ghcr.io/guardana/guardana:0.26` | the CLI — `scan`, `probe`, `monitor`, `diff` and the rest | `guardana` |
| `ghcr.io/guardana/guardana-collector:0.26` | the optional collector | `guardana-collector` |
...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.26 scan /work
```
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.26 \
...
docker run --rm --user "$(id -u):$(id -g)" -v "$PWD:/work:ro" \
  ghcr.io/guardana/guardana:0.26 scan /work
```
...
docker run --rm \
  ...
  ghcr.io/guardana/guardana-collector:0.26 migrate

docker run -d --name guardana-collector -p 8000:8000 \
  -e GUARDANA_DATABASE_URL="postgresql://guardana:...@db:5432/guardana" \
  ghcr.io/guardana/guardana-collector:0.26
```
