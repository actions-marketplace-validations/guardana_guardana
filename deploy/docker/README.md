...
|---|---|---|
| `ghcr.io/guardana/guardana:0.25` | the CLI — `scan`, `probe`, `monitor`, `diff` and the rest | `guardana` |
| `ghcr.io/guardana/guardana-collector:0.25` | the optional collector | `guardana-collector` |
...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/guardana/guardana:0.25 \
...
docker run --rm --user "$(id -u):$(id -g)" -v "$PWD:/work:ro" \
  ghcr.io/guardana/guardana:0.25 scan /work
```
