...
jobs:
  guardana:
    ...
    steps:
      ...
      - uses: guardana/guardana@v0.25   # moving tag; pins to the latest 0.25.x
        with:
          path: .
...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.25 \
...
```yaml
include:
  - remote: "https://raw.githubusercontent.com/guardana/guardana/v0.25/deploy/ci/gitlab-ci.yml"
