...
jobs:
  guardana:
    ...
    steps:
      ...
      - uses: guardana/guardana@v0.26   # moving tag; pins to the latest 0.26.x
        with:
          path: .
...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.26 \
...
```yaml
include:
  - remote: "https://raw.githubusercontent.com/guardana/guardana/v0.26/deploy/ci/gitlab-ci.yml"
